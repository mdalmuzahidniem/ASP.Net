...
namespace DAL
{
    public class CompletedWorkRepo : IRepository<Order_Details, int,string>
    {
        Service_Entities db;
        public CompletedWorkRepo(Service_Entities db)
        {
            this.db = db;
        }
        ...
        public List<Order_Details> GetDetails(int id)
        {
            var e = (from d in db.Order_Details
                     where d.employee_id == id && d.Order.status == "Completed"
                     select d).ToList();
            return e;
        }
    }
